...
const Shortlisted = ()=>{
    ...
    const uniqueWorkExps = [...new Set(shortlisted.map((app) => app.workExp))];
    const uniqueLocations = [...new Set(shortlisted.map((app) => app.location))];
    const uniqueSources = [...new Set(shortlisted.map((app) => app.source))];
    
     useEffect(() => {
    const data = JSON.parse(localStorage.getItem("shortlistedApplications")) || [];
    setShortlisted(data);
  }, []);
 
    
    useEffect(() => {
      let filtered = [...shortlisted];
    ...
    }, [workExpFilter, locationFilter, sourceFilter, selectedDateRange,shortlisted]);
  ...
  const updatedRejected = [
    ...
    {
      ...application,
      rejectedDate: new Date().toISOString().split("T")[0], // add rejected date
    },
  ];
  ...
  const updatedList = shortlisted.filter((item) => item.id !== application.id);
  setShortlisted(updatedList);
  setFilteredData(updatedList);
  localStorage.setItem("shortlistedApplications", JSON.stringify(updatedList));
};

     return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
       <h2 className="text-xl font-bold mb-4">Shortlisted Applications</h2>
        {/* Filter dropdowns */}
