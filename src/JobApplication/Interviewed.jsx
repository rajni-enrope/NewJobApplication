...
const [interviewData, setInterviewData] = useState([]);  
  ...
  const uniqueWorkExps = [...new Set(interviewData.map((app) => app.workExp))];
  const uniqueLocations = [
    ...new Set(interviewData.map((app) => app.location)),
  ];
  const uniqueSources = [...new Set(interviewData.map((app) => app.source))];

  useEffect(() => {
    const stored =
      JSON.parse(localStorage.getItem("interviewedApplications")) || [];
    setInterviewData(stored);
  }, []);
  ...
  const handleShortlist = (application) => {
    const shortlisted =
      JSON.parse(localStorage.getItem("shortlistedApplications")) || [];
    ...
    const updatedShortlisted = [...shortlisted, applicationWithDate];
    localStorage.setItem(
      "shortlistedApplications",
      JSON.stringify(updatedShortlisted)
    );
    // Remove from Interviewed list
    const updatedInterviewed = interviewData.filter(
      (item) => item.id !== application.id
    );
    setInterviewData(updatedInterviewed);
    setFilteredData(updatedInterviewed);

    localStorage.setItem(
      "interviewedApplications",
      JSON.stringify(updatedInterviewed)
    );
  };
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold mb-4">Interviewed Applications</h2>
        {/* Filter dropdowns */}
          ...
          <tbody>
            {filteredData.map((app) => (
              <tr key={app.id}>
                ...
                <td className="p-2 border border-gray-300">
                  {app.acceptedDate
                    ? new Date(app.acceptedDate).toLocaleDateString()
                    : "-"}
                </td>
                ...
                <td className="p-2 border  border-gray-300">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="border border-green-300 text-green-500 px-2 py-2 text-xs rounded  hover:bg-blue-100"
                    >
                      Shortlist
                    </button>
                    <button
